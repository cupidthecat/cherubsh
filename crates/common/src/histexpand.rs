//! Bash-style history expansion for forms such as `!!`, `!n`, and `^old^new^`.
//!
//! Expansion happens on the raw input line before lexing, as it does in Bash.
...
fn expand_reference(
    ...
    while pos < chars.len() && chars[pos] == ':' {
        ...
        match apply_modifier(chars, pos, &mut selection) {
            ...
            Err(error) => {
                let event: String = chars[bang..start + event_consumed].iter().collect();
                let error = error
                    .strip_prefix('!')
...
mod tests {
    ...
    #[test]
    fn invalid_modifier_reports_the_full_event_designator() {
        let mut history = HistoryTable::new(10);
        history.add("echo old", HistControl::empty());

        let result = expand("!!:z", &history);
        assert_eq!(
            result.error.as_deref(),
            Some("!!:z: history expansion failed")
        );
    }
