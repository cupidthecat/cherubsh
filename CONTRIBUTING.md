...
The full driver always runs the vendored Oils OSH corpus. Its ratchet is strict: `FAIL`, `DRIFT`, `XPASS`, and `STALE` are all test failures. Do not replace `crates/test-harness/oils-known-mismatches.tsv` just to make a change pass. Inspect the raw files under `target/parity/oils/failures`, confirm that each changed observation is intentional, and keep the ratchet diff in the same focused compatibility change. The generated `suggested-ratchet.tsv` is a review aid, not an automatic approval.
